...
const partialProduct: OptionalProduct = { id: 2 };

type ProductWithColor = OptionalProduct & { color?: string };

type ProductWithColor2 = Partial<Product & { color: string }>;
...
// SummaryCard = { id: string, name: string }
...
// PromoProduct = { id: string, name: string, category: string }
...
type ExtendedProduct = Product & { type: "physical" | "digital" };
...
type ProductWithRole<R extends Role> = R extends "Customer"
  ? Omit<Product, "price">
  : Product;
...
type UserWithRole<R extends Role> = R extends "Admin"
  ? User
  : Omit<User, "superUser">;
...
type UIUser = Pick<User, "name" | "role">;
